...
func (service *BotService) RegisterUserWithemail(db *sql.DB, app *config.App, bot *tb.Bot, m *tb.Message, lastState *models.UserLastState, text string, userID int) {
...
func (service *BotService) ConfirmRegisterCompanyRequest(db *sql.DB, app *config.App, bot *tb.Bot, m *tb.Message, lastState *models.UserLastState, text string, userID int) {
	userModel := new(tb.User)
	userModel.ID = userID
	switch text {
	case "Yes":
		...
		SaveUserLastState(db, app, bot, "", userID, "join_request_added")
		bot.Send(userModel, "Your Request Sent To The Admin And Will be Active After Confirmation.", HomeKeyOption(db, app))
	case "No":
		SaveUserLastState(db, app, bot, "", userID, "join_request_dismissed")
...
func (service *BotService) ConfirmRegisterUserForTheCompany(db *sql.DB, app *config.App, bot *tb.Bot, m *tb.Message, lastState *models.UserLastState, text string, userID int) {
	userModel := new(tb.User)
	userModel.ID = userID
	switch text {
	case "Yes":
		...
		SaveUserLastState(db, app, bot, lastState.Data, userID, "email_for_user_registration")
		bot.Send(userModel, "Please Enter The Code That Sent To Your Email Address", HomeKeyOption(db, app))
	case "No":
		SaveUserLastState(db, app, bot, "", userID, "cancel_user_registration_for_the_company")
		bot.Send(userModel, "You Can Continue in Bot by Pressing The Home Button", HomeKeyOption(db, app))
	}
}
